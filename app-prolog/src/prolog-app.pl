...
hotel('hilton',[wifi,piscina,estacionamiento,spa,restaurante,bar,gym],'la altagracia',alta,3,4284).
hotel('casa de campo',[wifi,alberca,spa,restaurante,bar,gym],'la romana',excelente,5,206).
hotel('hyatt',[piscina,spa,restaurante,bar,gym],'la altagracia',excelente,5,511).
hotel('gran jimenoa',[piscina,spa,estacionamiento,mascotas,restaurante,bar],'la vega',alta,2,52).
...
cerca('la altagracia','la romana').
cerca('la altagracia','el seibo').
cerca('la romana','san pedro de macoris').
cerca('la romana','el seibo').
cerca('san pedro de macoris','hato mayor').
cerca('san pedro de macoris','santo domingo').
cerca('santo domingo','monte plata').
cerca('monte plata','samana').
cerca('samana','duarte').
cerca('duarte','maria trinidad sanchez').
cerca('hermanas mirabal', 'espalliat').
cerca('espalliat','santiago').
cerca('santiago','puerto plata').
cerca('santiago','valverde').
cerca('valverde','montecristi').
cerca('montecristi','dajabon').
cerca('montecristi','santiago rodriguez').
cerca('dajabon','elias piña').
cerca('elias piña','san juan').
cerca('elias piña','san juan').
cerca('elias piña','independencia').
cerca('independencia','bahoruco').
cerca('independencia','pedernales').
cerca('pedernales','barahona').
cerca('barahona','azua').
cerca('san jose de ocoa','azua').
cerca('san jose de ocoa','peravia').
cerca('san jose de ocoa','monseñor nouel').
cerca('sanchez ramirez','monseñor nouel').
cerca('san cristobal','monte plata').
cerca('san cristobal','peravia').
cerca('santo domingo','monte plata').
cerca('distrito nacional', 'santo domingo').
...
searchRestaurante(Name,Location,FoodType,BudgetType,Stars):-
    local(Name,restaurante,Location,Stars),tipoComida(Name,FoodType,Price),clasificacionPrecio(Price,Type), Type = BudgetType,total(X).
...
checkHoteles(Nombre, Servicios, Ubicacion, Valoracion,Estrellas,TipoPrecio):-
                                     ...
                                     subset(Servicios,ServiciosHotel),total(X).
...
getActividades(Tipo,Ubicacion,TPrecio,Result):-findall([Nombre,Tipo,Ubicacion,Precio],
                                               ...
                                               Result).
auxActividades(Nombre,Tipo,Ubicacion,TPrecio,Precio):- actCultural(Nombre,Tipo,Ubicacion,Precio),clasificacionPrecio(Precio,TPrecio),total(X).
